...
import { fetchPygToBrlRate } from '../services/bcbExchangeRate';
...
const MacroDashboard: React.FC<Props> = ({ accounts, recurringPayments, onEdit, onInstallments }) => {
  ...
  const [pygRate, setPygRate] = useState<number | null>(null);
  ...
  useEffect(() => { fetchPygToBrlRate().then(({ rate, date }) => { setPygRate(rate); setRateDate(date); }).catch(() => setRateError(true)); }, []);
  const totalPyg = pygOne + pygTwo;
  return <div className="space-y-8">
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
      ...
      <div><DashboardCard title="Ueno" amount={pygOne} icon={<BankIcon />} color="text-cyan-400" currency="PYG" />{pygRate && <p className="text-xs text-zinc-500 mt-2 px-2">≈ {money(pygOne * pygRate, 'BRL')}</p>}</div>
      <div><DashboardCard title="BNF" amount={pygTwo} icon={<BankIcon />} color="text-blue-400" currency="PYG" />{pygRate && <p className="text-xs text-zinc-500 mt-2 px-2">≈ {money(pygTwo * pygRate, 'BRL')}</p>}</div>
      <DashboardCard title="Nubank Card Debt" amount={card} icon={<CreditCardIcon />} color="text-rose-400" currency="BRL" />
    </div>
    <div className="bg-zinc-800/60 p-5 rounded-2xl flex flex-col md:flex-row md:items-center justify-between gap-3"><div><p className="text-zinc-400 text-sm">Paraguayan accounts in BRL</p><p className="text-2xl font-bold text-cyan-300">{pygRate ? money(totalPyg * pygRate, 'BRL') : 'Loading rate…'}</p></div><p className="text-xs text-zinc-500">{rateError ? 'Could not load Banco Central rate.' : pygRate ? `BCB SML rate · 1 PYG = ${pygRate.toFixed(7)} BRL · ${rateDate}` : 'Loading Banco Central do Brasil rate'}</p></div>
